    ...
    <div style={{ minHeight: '100vh', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <main style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 24px' }}>
          ...
          {/* Main Game Area */}
<div>
  {/* Row 1: Dice card + Bet/Connect card — equal height */}
  <div
    style={{
      display: 'grid',
  ...
  >
      ...
      >
          ...
          <div>
            ...
            {loading ? (
              ...
              <motion.div
                ...
                initial={{ scale: 1.1 }}
                animate={{ scale: 1 }}
                style={{ fontSize: '2rem', fontWeight: 900, color: '#f97316', letterSpacing: '-0.03em', textShadow: '0 0 20px rgba(249,115,22,0.4)' }}
              >
                {potUCT.toFixed(2)}
        ...
        <div style={{ width: '100%' }}>
          {loading ? (
            ...
            <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ fontSize: '0.65rem', color: '#555', letterSpacing: '0.1em', flexShrink: 0 }}>
                COMMIT HASH
              </div>
              <code style={{ fontSize: '0.65rem', color: '#f97316', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
    ...
    {/* Right: Connect or Bet Form — same row, stretched to match dice height */}
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {!connected ? (
        <motion.div
          ...
          className="card-glass"
          style={{ borderRadius: 24, padding: 32, textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}
        >
          <div style={{ fontSize: '3rem', marginBottom: 16 }}>🔐</div>
          <h2 style={{ fontSize: '1.3rem', fontWeight: 800, marginBottom: 8 }}>Connect to Play</h2>
          ...
          <AnimatePresence>
            {walletError && (
              <motion.div
                ...
                style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 10, padding: '10px 14px', fontSize: '0.8rem', color: '#ef4444', marginBottom: 16 }}
              >
                {walletError}
              </motion.div>
            )}
          ...
          <motion.button
            ...
            style={{ width: '100%', padding: '16px 24px', borderRadius: 14, fontSize: '1rem', fontWeight: 700, letterSpacing: '0.03em' }}
          >
            {connecting ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="card-glass"
            style={{ borderRadius: 16, padding: '14px 20px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}
          >
            <div style={{ width: 36, height: 36, background: 'linear-gradient(135deg, #f97316, #ea6a07)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', flexShrink: 0, boxShadow: '0 0 12px rgba(249,115,22,0.4)' }}>
              👤
            </div>
            <div>
              <div style={{ fontWeight: 700, color: '#f97316', fontSize: '0.95rem' }}>@{identity?.nametag}</div>
              <div style={{ fontSize: '0.7rem', color: '#555' }}>Sphere wallet connected</div>
            </div>
            <button
              onClick={disconnect}
              style={{ marginLeft: 'auto', background: 'transparent', border: '1px solid #444', color: '#999', fontSize: '0.75rem', padding: '4px 10px', borderRadius: 6, cursor: 'pointer' }}
            >
              Disconnect
            </button>
    ...
    {popupCountdown !== null && lastWinner && (
      ...
      >
        <motion.div
          ...
          style={{ background: '#161616', border: '1px solid rgba(249,115,22,0.4)', borderRadius: 24, padding: '40px 48px', textAlign: 'center', boxShadow: '0 0 60px rgba(249,115,22,0.3)' }}
        >
          ...
          <div style={{ fontSize: '0.85rem', color: '#666' }}>Next round starts in {popupCountdown}s</div>
        </motion.div>
      </motion.div>
    )}
  </AnimatePresence>
  ...
  <AnimatePresence>
    {settling && lastWinner && (
      <motion.div
        ...
        style={{ background: 'linear-gradient(135deg, rgba(249,115,22,0.15) 0%, rgba(234,106,7,0.08) 100%)', border: '1px solid rgba(249,115,22,0.4)', borderRadius: 20, padding: '24px 32px', textAlign: 'center', marginBottom: 24, boxShadow: '0 0 40px rgba(249,115,22,0.2)' }}
      >
        ...
        {roundState.bets.map((bet, i) => {
          ...
          return (
            <motion.div
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: isMe ? 'rgba(249,115,22,0.08)' : 'rgba(255,255,255,0.02)', border: `1px solid ${isMe ? 'rgba(249,115,22,0.3)' : 'rgba(255,255,255,0.05)'}`, borderRadius: 10 }}
            >
              <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ width: 80, height: 4, background: 'rgba(249,115,22,0.15)', borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${betPct}%`, background: 'linear-gradient(90deg, #f97316, #ea6a07)', borderRadius: 2 }} />
                </div>
  ...
  {/* Row 2: Recent Rounds (left, below dice) + Feature badges (right, below bet form) */}
  <div
    style={{
      display: 'grid',
      gridTemplateColumns: '1fr 340px',
      ...
      {recentRounds.length > 0 && (
        ...
        >
          ...
          {recentRounds.slice(0, 5).map((r) => (
            <div
              key={r.roundId}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.04)', fontSize: '0.8rem' }}
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {r.winnerNametag && (
                  <span style={{ color: '#f97316', fontWeight: 600 }}>@{r.winnerNametag}</span>
                )}
                <span
                  style={{
                    fontSize: '0.65rem', padding: '2px 8px', borderRadius: 6,
                    background: r.status === 'settled' ? 'rgba(34,197,94,0.1)' : r.status === 'cancelled' ? 'rgba(239,68,68,0.1)' : 'rgba(249,115,22,0.1)',
                    color: r.status === 'settled' ? '#22c55e' : r.status === 'cancelled' ? '#ef4444' : '#f97316',
